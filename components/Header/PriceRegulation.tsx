...
export default function PriceRegulation({ min, max }: { min: number, max: number }) {

  const router = useRouter()
  const [value, setValue] = useState<number[]>([220000, 3700000]);
  const dispatch = useDispatch()
  const filters = useAppSelector(state => state.fats.squareRoom)
  const numberRooms = useAppSelector(state => state.fats.numberOfRooms)

  useEffect(() => {
    dispatch(changePriceRoom({ price: [value[0], value[1]] }))
  }, [min, max, value]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const filter = {
        "f[square][min]":  filters[0],
        "f[square][max]":  filters[1],
        "f[price][min]":  value[0],
        "f[price][max]":  value[1],
        "f[rooms][]":  numberRooms
      }
